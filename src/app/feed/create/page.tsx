...
export default function CreatePostPage() {
  ...
  async function handleSubmit() {
    ...
    const insertData: any = {
      ...
      tags: tagsArr,
      data_sections:
        type === "data" ? dataSections.filter((d) => d.label && d.value) : [],
      poll_options:
        type === "poll"
          ? pollOptions
              .filter(Boolean)
              .map((o: string, i: number) => ({ id: i, text: o, votes: 0 }))
          : [],
    };
  ...
  return (
    <div className="max-w-xl mx-auto px-3 py-4">
      ...
      <div className="space-y-3">
        ...
        <input
          className="input"
          placeholder="Location (e.g. Annapurna Circuit)"
          value={location}
          onChange={(e) => setLocation(e.target.value)}
        />
        <input
          className="input"
          placeholder="Tags, comma separated (e.g. hiking, gear)"
          value={tags}
        ...
        {type === "data" && (
          <div className="space-y-2">
            ...
            {dataSections.map((ds, i) => (
              <div key={i} className="flex gap-2">
                ...
                {i > 0 && (
                  <button
                    onClick={() =>
                      setDataSections((prev) => prev.filter((_, j) => j !== i))
                    }
            ...
            <button
              onClick={() =>
                setDataSections((prev) => [...prev, { label: "", value: "" }])
              }
