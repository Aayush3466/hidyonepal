...
const CATEGORIES = ['tent','sleeping_bag','clothing','footwear','navigation','cooking','safety','other']
...
export default function ListEquipmentPage() {
  ...
  async function submit() {
    ...
    const { error: insertError } = await supabase.from('equipment').insert({
      ...
      price_per_day: listingType === 'rent' && pricePerDay
  ...
  return (
    <div className="max-w-xl mx-auto px-3 py-4">
      ...
      <div className="space-y-3">
        ...
        <textarea className="input min-h-[70px] resize-none" placeholder="Description"
          value={description} onChange={e => setDescription(e.target.value)} />

        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-earth-500 mb-1 block">Category</label>
          ...
          <div className="flex-1">
            <label className="text-xs text-earth-500 mb-1 block">Condition</label>
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-earth-500 mb-1 block">Listing Type</label>
            <select className="input" value={listingType}
          ...
          {listingType === 'rent' && (
            <div className="flex-1">
              <label className="text-xs text-earth-500 mb-1 block">Price/day (NPR)</label>
